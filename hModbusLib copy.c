...
void hModbusSwapU32DataByteArray(uint8_t* Data, uint8_t DataLength, hModbus32BitOrderTypeDef BitOrder){
  ...
  switch (BitOrder)
  {
  case hModbus32BitOrder_ABCD:
    break;
  case hModbus32BitOrder_DCBA:
    memcpy(TempData, Data, DataLength);
    for(uint8_t i=0; i<(DataLength/4); i++){
      Data[i*4+0] = TempData[i*4+3];
      Data[i*4+1] = TempData[i*4+2];
      Data[i*4+2] = TempData[i*4+1];
      Data[i*4+3] = TempData[i*4+0];
    }
    break;
  case hModbus32BitOrder_BADC:
    memcpy(TempData, Data, DataLength);
    for(uint8_t i=0; i<(DataLength/4); i++){
      Data[i*4+0] = TempData[i*4+1];
      Data[i*4+1] = TempData[i*4+0];
      Data[i*4+2] = TempData[i*4+3];
      Data[i*4+3] = TempData[i*4+2];
    }
    break;
  case hModbus32BitOrder_CDAB:
    memcpy(TempData, Data, DataLength);
    for(uint8_t i=0; i<(DataLength/4); i++){
      Data[i*4+0] = TempData[i*4+2];
      Data[i*4+1] = TempData[i*4+3];
      Data[i*4+2] = TempData[i*4+0];
      Data[i*4+3] = TempData[i*4+1];
    }
    break;
  default:
    break;
...
bool hModbusReadInputRegisters16i(hModbusTypeDef* Handle, uint8_t SlaveAddress, uint16_t StartNumber, uint16_t Length, uint16_t *Data){
  ...
  if(Ret == true)
  {
    hModbusSwapU16DataByteArray((uint8_t*)Data, Length, Handle->ByteOrder16);
    return true;
  }
  else
    return false;
...
bool hModbusReadInputRegisters32f(hModbusTypeDef* Handle, uint8_t SlaveAddress, uint16_t StartNumber, uint16_t Length, float *Data){
  ...
  if(ret == true)
  {
    hModbusSwapU32DataByteArray((uint8_t*)Data, Length, Handle->ByteOrder32);
    return true;
  }
  else
    return false;
